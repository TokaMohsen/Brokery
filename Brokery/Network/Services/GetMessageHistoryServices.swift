...
class GetMessageHistoryServices {
    func fetch(params : JSON , method : RequestMethod , url : String , completion: @escaping ([msgDto]?, WebError<CustomError>?) -> ())
    {
        let getListOfMessagesTask: URLSessionDataTask!
        
        var userMessageList = Resource< [msgDto] , CustomError>(jsonDecoder: JSONDecoder(), path: url, method: .get)
